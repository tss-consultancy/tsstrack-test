...
<html>
<head>
    <title>Committee Meeting Notification</title>
...
<p>As part of our ongoing commitment, I am thrilled to invite you to our upcoming Committee Meeting, which will be held on <strong>{{ $meeting->date_of_meeting }}</strong> from 
<strong>{{ $meeting->committee_meeting_start_time ? \Carbon\Carbon::parse($meeting->committee_meeting_start_time)->format('g:i A') : 'N/A' }}</strong> 
to 
<strong>{{ $meeting->committee_meeting_end_time ? \Carbon\Carbon::parse($meeting->committee_meeting_end_time)->format('g:i A') : 'N/A' }}</strong> 
